...
const storedRootValidity = time.Second
...
func (wfs *WFS) maybeLoadEntry(fullpath util.FullPath) (*filer_pb.Entry, entryVersion, fuse.Status) {
	...
	if string(fullpath) == wfs.option.FilerMountRootPath {
		return wfs.rootEntry(name), entryVersion{}, fuse.OK
	}
...
func (wfs *WFS) storedRootEntry() *filer_pb.Entry {
	...
	if wfs.storedRootFetched.IsZero() || time.Since(wfs.storedRootFetched) > storedRootValidity {
		entry, _, _, err := filer_pb.GetEntry(context.Background(), wfs, util.FullPath(wfs.option.FilerMountRootPath))
	...
	if wfs.metaCache.IsDirectoryCached(dirPath) && wfs.metaCache.IsNameFresh(fullpath) {
		...
		if wfs.metaCache.IsDirectoryCached(dirPath) && wfs.metaCache.IsNameFresh(fullpath) {
			if handleEntry := wfs.lookupDirtyHandleEntry(fullpath); handleEntry != nil {
				...
				return handleEntry, entryVersion{}, fuse.OK
			}
