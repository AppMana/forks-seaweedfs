...
func TestVolumeFsckHttpDeleteRecordsParentDirectory(t *testing.T) {
	...
	if err := c.httpDelete("/orphan/dir/deep/file.txt"); err != nil {
		t.Fatalf("successful delete: %v", err)
	}
	if err := c.httpDelete("/locked/dir/file.txt"); err == nil {
		t.Fatal("rejected delete must return an error")
	}
...
func TestVolumeFsckFilerDeleteFailureMakesPurgeIncomplete(t *testing.T) {
	filer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	const (
		dataNodeId = "node-a"
		volumeId   = uint32(7)
		itemPath   = util.FullPath("/missing/chunk.txt")
	)
	if err := writeToFile(nil, getVolumeFileIdFile(tempFolder, dataNodeId, volumeId)); err != nil {
		t.Fatal(err)
	}
	record := make([]byte, readbufferSize)
	util.Uint64toBytes(record, 123)
	util.Uint32toBytes(record[8:], 456)
	util.Uint32toBytes(record[12:], uint32(len(itemPath)))
	record = append(record, []byte(itemPath)...)
	if err := writeToFile(record, getFilerFileIdFile(tempFolder, volumeId)); err != nil {
		t.Fatal(err)
	}
	...
	if err := c.oneVolumeFileIdsCheckOneVolume(dataNodeId, volumeId, true); err == nil {
		t.Fatal("filer DELETE status 500 must make the purge incomplete")
	}
}
