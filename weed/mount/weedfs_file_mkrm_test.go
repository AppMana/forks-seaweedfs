...
// bucketXattrPrefix is the filer's key prefix for user xattrs, spelled out
// here so this file builds on platforms without the xattr handlers.
const bucketXattrPrefix = "xattr-"
...
func newBucketRootEntry(name string) *filer_pb.Entry {
	return &filer_pb.Entry{
		...
		Extended: map[string][]byte{
			...
			bucketXattrPrefix + "user.owner":    []byte("ci"),
		},
		Quota: 42,
	}
}
...
func TestRootTouchPreservesBucketAttributes(t *testing.T) {
	...
	for _, update := range updates {
		...
		if got := string(entry.GetExtended()[bucketXattrPrefix+"user.owner"]); got != "ci" {
