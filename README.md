...
| `SEAWEEDFS_RELIABILITY_LABCONTAINERS_REF` | Full commit SHA for `AppMana/labcontainers` used to build `labd` and its Go SDK | Must include native topology objects, approved Plan/Apply recovery, raw execution requests, retained debugging leases, and the pinned Containerlab recovery build script. These APIs are currently in the isolated native-SDK migration worktree; publish and qualify its commit before selecting it in Actions. Never point this at a moving branch. |
