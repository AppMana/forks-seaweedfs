...
type countingNeedleMapper struct {
	NeedleMapper
	syncCount int
	syncErr   error
}

func (m *countingNeedleMapper) Sync() error {
	m.syncCount++
	return m.syncErr
}
...
// A durable acknowledgement covers the lookup mapping as well as the data
// append. Without an index fsync, a hard restart can expose the previous
// version even though the acknowledged bytes are present later in the .dat.
func TestWriteNeedle2FsyncsIndexBeforeAcknowledging(t *testing.T) {
	v, _ := newCountingVolume(t)
	counting := &countingNeedleMapper{NeedleMapper: v.nm}
	v.nm = counting

	_, _, _, err := v.writeNeedle2(newRandomNeedle(1), true, true, false)
	require.NoError(t, err)
	require.Equal(t, 1, counting.syncCount,
		"a durable write must flush the index mapping before acknowledgement")
}

func TestWriteNeedle2RejectsFailedIndexFsync(t *testing.T) {
	v, _ := newCountingVolume(t)
	indexErr := errors.New("index device lost")
	counting := &countingNeedleMapper{NeedleMapper: v.nm, syncErr: indexErr}
	v.nm = counting
	...
	require.ErrorIs(t, err, indexErr,
		"a write with an unflushed lookup mapping must not be acknowledged")
	require.Equal(t, 1, counting.syncCount)
	after, _, statErr := v.DataBackend.GetStat()
	require.NoError(t, statErr)
	require.Greater(t, after, before, "durable data must not be truncated after only the index sync fails")
	_, found := v.nm.Get(1)
	require.True(t, found, "the in-memory mapping must continue to describe the preserved append")
}
