...
func runMini(cmd *Command, args []string) bool {
	...
	*miniAdminOptions.dataDir = util.ResolvePath(*miniAdminOptions.dataDir)
	...
	pb.RegisterLocalGrpcSocket(*miniIp, *miniMasterOptions.portGrpc, filepath.Join(os.TempDir(), fmt.Sprintf("seaweedfs-master-grpc-%d.sock", *miniMasterOptions.portGrpc)))
